...
class TestImageAPI(TestCase):  # Test the private ingredients api
    ...
    def test_retrieve_images_list(self):
        test_image = SimpleUploadedFile('test_image.jpg',  b"image_content",  content_type='image/jpeg')
        x = Image.objects.create(title='Warner', creation_date=datetime.datetime(2922,1,19,14,30), image=test_image, submitter=self.user)
    ...
    def test_search_images_list(self):
        url = '{url}?{filter}={value}'.format(
            ...
            filter='search', value='opening')
        test_image = SimpleUploadedFile('test_image.jpg',  b"image_content",  content_type='image/jpeg')
        x = Image.objects.create(title='Warner', creation_date=datetime.datetime(2922,1,19,14,30), image=test_image, submitter=self.user)
        y = Image.objects.create(title='Khawaja', creation_date=datetime.datetime(2922,1,18,14,30), image=test_image, submitter=self.user)
        x.tags.add("opening", "batsman")
    ...
    def test_patch_existing_image(self):
        test_image = SimpleUploadedFile('test_image.jpg', b"image_content", content_type='image/jpeg')
        x = Image.objects.create(title='Warner', creation_date=datetime.datetime(2922, 1, 19, 14, 30), image=test_image,
                                 submitter=self.user)
        x.tags.add("opening", "batsman")
        print(IMAGES_URL + '1/')
